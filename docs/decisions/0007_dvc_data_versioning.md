...
- **Dónde corre `dvc add`/`dvc push`**: en `pipelines/data/download` ([ADR 0002](0002_pipelines_vs_src_separation.md)), el mismo punto que ya trae datos externos y ya anonimiza antes de escribir a `data/raw` ([docs/architecture/data_governance.md](../architecture/data_governance.md)). DVC no cambia esa responsabilidad, se inserta justo después.
