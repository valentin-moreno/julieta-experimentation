...
## Fase 2 (2026-09-09): `compute_features.py` y el loader de impedancia, para IDXX-healthy_model_groupsplit

Al traer el experimento real `VMV-healthy_model_experimentos_groupsplit` (ver `experiments/IDXX-healthy_model_groupsplit/README.md`), hizo falta calcular sus features (`feature_mode="advanced_nyquist"`). Mismo criterio que la fase 1: portar solo lo maduro y realmente usado, no el archivo completo.

- **`compute_features.py` origen tiene 3076 líneas y 9 bloques** (`compute_statistics`, `ComputeColeColeFeatures`, `ComputeNyquistPlotFeatures`, `ComputeAdvancedNyquistFeatures`, `ComputeAdvancedMagPhaseFeatures`, `ComputeBreastLevelStatistics`, `ComputeWaveletFeatures`, `MahalanobisDepthFeature`, `ComputeMarkovFeatures`). Se portaron solo 3: `ComputeColeColeFeatures` (modelo físico estándar, no experimental), `ComputeAdvancedNyquistFeatures` (lo que el experimento usa de verdad) y `ComputeBreastLevelStatistics` (agregación por seno). El resto: o está marcado por su propio docstring como "variante experimental" (`ComputeAdvancedMagPhaseFeatures`), o no tiene evidencia de uso real en ningún experimento de este repo (`ComputeWaveletFeatures`, `MahalanobisDepthFeature`, `ComputeMarkovFeatures`) — quedan fuera hasta que alguien traiga un experimento real que sí las necesite.
- **`MakeDataset` (2211 líneas) NO se portó como clase.** Solo hacía falta leer un `.h5` de resistencia/reactancia (`get_measurements_data`) y aplanar un dict a DataFrame (`flatten_data`) — se escribió eso de cero y scoped en `src/julieta/data/loaders/impedance_h5.py`, pero **leyendo el comportamiento real de esos métodos en el original para replicarlo exacto**, no inventándolo: en particular, `flatten_data` no deduplica por defecto, y el deduplicado real (`keep="last"` sobre el índice `(patient_id, side)`) pasa después, en `_load_advanced_nyquist_features`. Se verificó que esto sí importa: el `.h5` de CAFAM trae 72 duplicados de 228 filas. Se descartó importar la clase completa porque además de esos dos métodos trae dependencias de ploteo (`missingno`, `julieta.visualization.visualize.VisualizeData`) que no hacen falta para esto.
- **`pipelines/data/build_training_dataset.py` tampoco se portó tal cual.** Depende de `data/raw/disconnections.xlsx`, un archivo que no existe ni en este repo ni en julieta-models al momento de portar esto — correrlo tal cual hubiera fallado con `FileNotFoundError`. La lógica de labeling/exclusión (BI-RADS → clase, exclusión por desconexión, fix de `height`/`weight`==0, dedup por `(patient_id, side)`) se reescribió leyendo el comportamiento real del original, en [`pipelines/data/build/build_healthy_advanced_nyquist_dataset.py`](../../pipelines/data/build/build_healthy_advanced_nyquist_dataset.py) — un pipeline de este repo, no código dentro de la carpeta del experimento, siguiendo [ADR 0002](0002_pipelines_vs_src_separation.md).
